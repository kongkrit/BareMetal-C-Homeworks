    ...
    if (dom.savePDFBtn) {
        dom.savePDFBtn.addEventListener('click', () => {
            ...
            for (let i = 1; i <= numQuizzes; i++) {
                ...
                // Add page break style to the section itself for subsequent quizzes
                // using both legacy and modern properties
                const breakStyle = (i > 1) ? ' style="page-break-before: always; break-before: page;"' : '';
                
                combinedHtml += `<div class="quiz-section"${breakStyle}>${quizHtml}</div>`;
            }
